...
if (await bootWith('build-13 layout', B13, 's-menu')) {
  ...
  (ne && ne.v === 4 && Array.isArray(runs) && runs.every(r => r.v === 4)) ? ok('build-13 layout: migrated to `ne` v4, runs stamped RUN_SCHEMA 4') : bad('build-13 layout: ne v4 + run stamp', JSON.stringify({ v: ne && ne.v, stamps: runs && runs.map(r => r.v) }));
...
// longer scores in, so they retire — and nothing else does
{
  ...
  if (await bootWith('a v1 record carrying pre-build-31 Timing runs', OLD, 's-menu')) {
    ...
    (ne.v === 4 && kinds.join('|') === want && ne.runs.every(r => r.v === 4) && ne.bars['timing:hidden:10'] && ne.bars['quick-tap:two:5'] && ne.ach.tm_wall && ne.ach.first)
...
console.log('\nbuild 35 - batch 15, bugs and the runs');
{
     ...
     and one that never ran is rebuilt by the next tap. Nobody can hear any of it here — the phone is the check. ---- */
  {
    const has = {
      ...
      foreground: /visibilitychange',\(\)=>\{ if\(!document\.hidden&&ac&&ac\.state!=='running'\) revive\('foreground'\)/.test(audio35),
      pageshow: /addEventListener\('pageshow'/.test(audio35),
      tap: /pointerdown',\(\)=>\{ if\(ac&&ac\.state!=='running'\) revive\('tap',true\)/.test(audio35),
    ...
    /^audio · (running|suspended|interrupted|closed|none) · context \d+ · rebuilt · tap/.test(ro)
  ...
  /* ---- F.4: no player colour where customisation lives; previews white until a colour is chosen for that game ---- */
  {
    ...
    await setStorage({ ne: { v: 3, prefs: { ...OPEN_PREFS, allOpen: false, col: COLS }, runs: [{ t: NOW35, g: 'quick-tap', d: 'two', s: 5, n: '', v: 4, hits: 10, misses: 0 }, { t: NOW35 - 1, g: 'dots', d: 'blind', s: 5, n: '', v: 4, hits: 8, misses: 0 }], ach: {}, unlock: { 'dots:blind': NOW35 }, intro: SEEN_INTRO, seen: {}, bars: {} } });
    await page.reload({ waitUntil: 'networkidle0' }); await sleep(400);
  ...
  /* ---- D.9, D.10, #415 and the Verdict Desk data edit ---- */
  {
    ...
    const lineBad = [];
    for (const [k, tiers] of Object.entries(LINES)) for (const [t, want] of Object.entries(tiers)) if ((T35[k].lines[t] || []).join('|') !== want.join('|')) lineBad.push(k + ':' + t);
    const halfTyped = Object.values(T35).flatMap(r => Object.values(r.lines).flat()).filter(l => /^Almost a\s*$|^Ooft/.test(l));
    (!lineBad.length && !halfTyped.length && T35.sequence.lines.ace[0] === 'Very long memory.')
      ? ok('Verdict Desk: Aiden\'s lines for Quick Tap, Dots, Grow and Cut are in; the two half-typed lines are not; the four Grow lines he did not edit keep build 34\'s; Sequence is untouched') : bad('Verdict Desk lines', JSON.stringify({ lineBad, halfTyped }));
    ...
    (split.every(k => T35[k] && T35[k].at.join() === '0.85,0.6,0.35') && !T35.timing && !T35.reaction && same('timing:stopwatch', 'timing:hidden') && same('reaction:flash', 'reaction:nogo') && T35['timing:hidden'].lines.bad[0] === 'Way off. Count it out loud.' && T35['reaction:nogo'].lines.ace[0] === 'Very quick.')
      ? ok('D.10 Timing and Reaction are keyed per mode - four rows seeded from their parents, the parents gone, and Timing\'s numbers are still build 34\'s (#414)') : bad('D.10 the split', JSON.stringify(Object.keys(T35)));
