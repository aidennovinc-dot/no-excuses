...
| `keys` | the keys, the surface and #375 (v15 sections 5 and 6) | v15 §5.1–5.4 and §6.1–6.5; #375a–c two-player defects; v26 items 3, 9, 10, 11, 12 — the green menu items (earned through play), the key cards and count line, all three key reveals to their last frame, the key asking before its chest opens (Aiden's answer after build 48), the version label on the home menu only (build 48); v26 items 2 and 4 (build 49) — the map's first open drawn out to 6–8s in its order, once, replayed by Fresh game; an unwatched clip pulses on About |
| `button actions` | button actions (every data-act at least once) | every `data-act` control driven at least once |
| `chests` | chests | v26 items 5, 6, 8, 12 and 13 (build 49) — all four chests' rewards fly out of the lid one at a time and settle under the chest, in colour, the About video among them, the name and "tap to continue" after the last lands, the card below them; the two Gauntlet tiles and their placeholder screen. v26 items 7 and 12 (build 48) — one saved value: the Games chest and key 1 EARNED BY RUNS and reached by Testing's buttons land in the same state, and after every step the store, the map, the Keys screen and the menu agree on a state play can reach; the 0–300 meter, the Pro chest at 200, locked chests in words, no percentage on the Games chest |
| `music` | music | v26 §B1 (build 49) — the four result sounds climb and a round's is one note shorter with bass; End of run lands before the tier plays; seven whoosh versions at random and longer title whooshes; Sigh held; the Key chest from key 1's theme, the Pro chest and Thorn earned bigger |
