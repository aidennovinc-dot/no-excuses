...
const RX=Object.assign(roundEngine(),{ id:'reaction', times:[], faults:0, t0:0, rule:'circle', shown:'', armed:false, dry:0, over:0, out:false, wrong:0, seen:0, vsN:[0,0], vsDone:false,
  ...
  result(){ const [x,y]=minMax(this.times); if(this.streak()) return {hits:this.times.length,misses:this.faults,x,y,lim:'500ms'}; return {hits:this.times.length?Math.round(mean(this.times)):0,misses:this.faults,x,y}; },
  ...
  noTap(){ const ms=600; this.st='show'; this.times.push(ms); this.over+=Math.max(0,ms-200); if(this.over>=500) this.out=true;
    ...
    pane.innerHTML=rxBox()+`<div class="rxmsg">${CP.noTap}${this.out?CP.reached:''}<b>600<small style="font-size:14px;letter-spacing:.2em">${CP.ms}</small></b></div>`; this.ctx.audio.miss(); if(navigator.vibrate) navigator.vibrate(30); this.hud(); this.ctx.emit('live',this.result()); this.later(()=>this.next(),1400); },
  onDown(ev){ if(this.versus()) return this.vsTap(ev); if(this.nogo()) return this.nogoTap(ev);
    ...
    const ms=Math.max(1,Math.round(ev.t-this.t0)); this.st='show'; this.times.push(ms); if(this.streak()){ this.over+=Math.max(0,ms-200); if(this.over>=500) this.out=true; }
    hud.score(this.streak()?String(this.times.length):Math.round(mean(this.times))); const pane=$('#rxpane'); pane.classList.remove('lit'); pane.classList.add('hit'); pane.innerHTML=rxBox()+`<div class="rxmsg">${ms<200?CP.quick:ms<300?CP.good:CP.slowWord}${this.out?CP.reached:''}<b>${ms}<small style="font-size:14px;letter-spacing:.2em">${CP.ms}</small></b></div>`; this.ctx.audio.hit(); this.hud(); this.ctx.emit('live',this.result()); this.later(()=>this.next(),1200); },
  // a fault (v9) is big and stays 1.7s — "missed it · again" used to be small type gone in under a second
  fault(msg){ this.st='fault'; this.clearT(); const pane=$('#rxpane'); pane.classList.remove('lit'); pane.classList.add('bad'); pane.innerHTML=`<div class="rxmsg" style="top:30%"><b class="fb">${msg}</b><span class="sub">${T(CP.again,{n:this.round,of:this.streak()?'':T(CP.of,{s:this.ctx.len})})}</span></div>`; this.ctx.audio.miss(); if(navigator.vibrate) navigator.vibrate(40); this.later(()=>this.again(),1700); },
  ...
  hudNogo(){ hud.time(this.streak()?T(CP.hudNogoStreak,{n:this.seen+1,w:this.wrong}):T(CP.hudNogo,{n:Math.min(this.ctx.len,this.seen+1),s:this.ctx.len,w:this.wrong})); },
  nogoScore(){ const [x,y]=minMax(this.times); if(this.streak()) return {hits:this.seen,misses:this.wrong,x,y,lim:'3 wrong'}; return {hits:Math.round((this.times.length?mean(this.times):600)+150*this.wrong),misses:this.wrong,x,y}; },
  ...
  nogoTap(ev){ if(this.st==='rule'||this.st==='rule2'||this.st==='over') return; const pane=$('#rxpane');
    if(this.st==='go'&&this.armed){ const ms=Math.max(1,Math.round(ev.t-this.t0)); this.times.push(ms); this.st='hit'; pane.innerHTML=`<div class="rxmsg" style="top:40%"><b style="font-size:clamp(28px,9vw,60px)">${ms}<small style="font-size:12px;letter-spacing:.2em">${CP.ms}</small></b></div>`; this.ctx.audio.hit(); hud.score(this.streak()?this.seen:Math.round(mean(this.times)+150*this.wrong)); this.ctx.emit('live',this.nogoScore()); return; }
    ...
    this.wrong++; this.st='wrongshow'; pane.classList.add('bad'); pane.innerHTML=`<div class="rxmsg" style="top:40%"><b class="fb" style="font-size:clamp(18px,6vw,36px)">${T(CP.wrong,{n:this.wrong})}</b></div>`; this.ctx.audio.miss(); if(navigator.vibrate) navigator.vibrate(40); hud.score(this.streak()?this.seen:Math.round((this.times.length?mean(this.times):600)+150*this.wrong)); this.hudNogo();
    if(this.wrong>=3){ this.clearT(); this.st='over'; hud.shake(); pane.innerHTML=`<div class="rxmsg" style="top:40%">${CP.three}<b style="font-size:28px">${CP.over}</b></div>`; return this.later(()=>this.nogoEnd(true),1300); } },
