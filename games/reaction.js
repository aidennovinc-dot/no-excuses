...
import { arm, finish } from "../app.js";
import { Snd } from "../audio.js";
import { $, $$, SHAPE_WORD, STREAK, mean, pWho, shapeI, vmin } from "../core.js";
...
const RX=Object.assign(roundEngine(),{ times:[], faults:0, t0:0, rule:'circle', shown:'', armed:false, dry:0, over:0, out:false, wrong:0, seen:0, vsN:[0,0], vsDone:false,
  ...
  hud(){ $('#hud-time').textContent=this.streak()?`attempt ${this.round} · ${Math.round(this.over)} of 500ms`:`${this.round} / ${sel.secs}`; },
  ...
  onDown(e){ if(this.versus()) return this.vsTap(e); if(this.nogo()) return this.nogoTap(e);
    ...
    $('#score').textContent=this.streak()?String(this.times.length):Math.round(mean(this.times)); const pane=$('#rxpane'); pane.classList.remove('lit'); pane.classList.add('hit'); pane.innerHTML=rxBox()+`<div class="rxmsg">${ms<200?'quick':ms<300?'good':'slow'}${this.out?' · 500 reached':''}<b>${ms}<small style="font-size:14px;letter-spacing:.2em"> ms</small></b></div>`; Snd.hit(); this.hud(); liveCheck(this.result()); this.later(()=>this.next(),1200); },
  // a fault (v9) is big and stays 1.7s — "missed it · again" used to be small type gone in under a second
  fault(msg){ this.st='fault'; this.clearT(); const pane=$('#rxpane'); pane.classList.remove('lit'); pane.classList.add('bad'); pane.innerHTML=`<div class="rxmsg" style="top:30%"><b class="fb">${msg}</b><span class="sub">try again · attempt ${this.round}${this.streak()?'':' of '+sel.secs}</span></div>`; Snd.miss(); if(navigator.vibrate) navigator.vibrate(40); this.later(()=>this.again(),1700); },
  ...
  vsRound(){ this.clearT(); this.round++; const need=Math.ceil(sel.secs/2); if(this.vsN[0]>=need||this.vsN[1]>=need||this.round>sel.secs) return this.vsEnd();
    $('#hud-time').textContent=`round ${this.round} · best of ${sel.secs}`; this.st='wait'; this.armed=false;
    $('#gen').innerHTML=`<div class="rxpane" id="rxpane"><div class="rxmsg" id="rxmsg" style="top:44%;font-size:11px">wait for it</div></div><div class="vz top p2">${pWho(1)}<b>${this.vsN[1]}</b></div><div class="vz bot p1">${pWho(0)}<b>${this.vsN[0]}</b></div>`;
  ...
  vsTap(e){ if(this.st!=='wait'&&this.st!=='go') return; const r=genRect(); const p=(e.clientY-r.top)<r.height/2?1:0; const early=this.st==='wait'; const w=early?1-p:p; this.st='show'; this.clearT(); this.vsN[w]++;
    ...
    pane.innerHTML=`<div class="rxmsg" style="top:40%"><b class="fb ${w?'p2':'p1'}" style="font-size:clamp(18px,5vw,30px)">Player ${w+1} takes it</b><span class="sub">${early?`Player ${p+1} tapped early`:ms+' ms'}</span></div>`; $$('.vz b')[w?0:1].textContent=this.vsN[w]; early?Snd.miss():Snd.hit(); this.later(()=>this.vsRound(),1500); },
  vsEnd(){ const [a,b]=this.vsN; const w=a>b?0:b>a?1:-1; this.st='over'; $('#gen').innerHTML=`<div class="rxpane"><div class="rxmsg" style="top:40%"><b class="fb ${w<0?'':w?'p2':'p1'}" style="font-size:clamp(18px,5vw,30px)">${w<0?'draw':'Player '+(w+1)+' wins'}</b><span class="sub">${a} – ${b}</span></div></div>`; Snd.end(); this.later(()=>finish({hits:a,misses:0,vs2:{a,b,w,how:`${a}–${b}`}}),1600); },
  ...
  hudNogo(){ $('#hud-time').textContent=this.streak()?`shape ${this.seen+1} · ${this.wrong} of 3 wrong`:`${Math.min(sel.secs,this.seen+1)} / ${sel.secs} · ${this.wrong} of 3 wrong`; },
  ...
  rulePause(){ this.clearT(); this.st='rule'; $('#gen').innerHTML=`<div class="rxpane" id="rxpane"></div>`; rxBar(['tap','only','the',shapeI(this.rule),`<b>${SHAPE_WORD[this.rule]}</b>`]); this.later(()=>this.beat(),2200); },
  beat(){ if(!this.streak()&&this.seen>=sel.secs) return this.nogoEnd(); if(this.seen>0&&this.seen%5===0&&this.ruleAt!==this.seen){ this.ruleAt=this.seen; this.st='rule2'; const others=['circle','square','tri'].filter(s=>s!==this.rule); this.rule=others[rnd(2)]; $('#rxpane').innerHTML=''; rxBar(['now','only','the',shapeI(this.rule),`<b>${SHAPE_WORD[this.rule]}</b>`]); return this.later(()=>{ this.st='gap'; this.beat(); },2200); }
  ...
  nogoTap(e){ if(this.st==='rule'||this.st==='rule2'||this.st==='over') return; const pane=$('#rxpane');
    ...
    this.wrong++; this.st='wrongshow'; pane.classList.add('bad'); pane.innerHTML=`<div class="rxmsg" style="top:40%"><b class="fb" style="font-size:clamp(18px,6vw,36px)">wrong tap · ${this.wrong} of 3</b></div>`; Snd.miss(); if(navigator.vibrate) navigator.vibrate(40); $('#score').textContent=this.streak()?this.seen:Math.round((this.times.length?mean(this.times):600)+150*this.wrong); this.hudNogo();
    if(this.wrong>=3){ this.clearT(); this.st='over'; const g=$('#game'); g.classList.remove('shake'); void g.offsetWidth; g.classList.add('shake'); pane.innerHTML=`<div class="rxmsg" style="top:40%">three wrong taps<b style="font-size:28px">run over</b></div>`; return this.later(()=>this.nogoEnd(true),1300); } },
