...
const TM=Object.assign(roundEngine(),{ errs:[], target:0, t0:0, ball:null, targets:[], out:false, tot:0,
  ...
  budget(){ return this.hid()?100:2; }, budTxt(){ return this.hid()?'100px':'2.0s'; }, totTxt(){ return this.hid()?Math.round(this.tot)+'px':f2(this.tot)+'s'; },
  ...
  hud(){ $('#hud-time').textContent=this.streak()?`attempt ${this.round} · ${this.totTxt()} of ${this.budTxt()}`:`${this.round} / ${sel.secs}`; },
  ...
  onDown(){ if(this.st!=='run') return; this.st='show'; cancelAnimationFrame(this.raf); const now=performance.now(); let err, note;
    ...
    $('#gen').insertAdjacentHTML('beforeend',`<div class="glbl bot" id="tmres"><b class="${good?'g':ok?'':'r'}" id="tmerr">${hid?err+'px':f2(err)+'s'}</b>${good?'dead on':ok?'close':note}</div>`); const h=$('#tmhint'); if(h) h.remove();
  ...
  addUp(err,hid){ const from=this.tot, t0=performance.now(), ms=800, el=$('#tmerr'); Snd.whoosh(ms,140,760);
    const step=now=>{ if(this.st!=='show') return; const k=Math.min(1,(now-t0)/ms); this.tot=from+err*k;
      ...
      $('#hud-time').textContent=`attempt ${this.round} · ${this.totTxt()} of ${this.budTxt()}`;
      if(k<1) requestAnimationFrame(step); else { this.tot=from+err; if(this.tot>=this.budget()) this.out=true; this.hud();
        if(this.out){ const r=$('#tmres'); if(r) r.insertAdjacentHTML('beforeend',`<br>${this.budTxt()} reached · run over`); }
