...
const timedEngine=()=>({ ctx:null, hits:0, misses:0, hitT:[], armed:false, lockUntil:0, meterAt:0, lockMs:CFG.lockout, hideOnMiss:true,
  mount(ctx){ this.ctx=ctx; Object.assign(this,{hits:0,misses:0,hitT:[],armed:false,lockUntil:0,meterAt:0}); this.reset(); this.render(false); },
  start(){ this.begin(); this.armed=true; this.arm(); },
  ...
  tick(ctx,now){ if(now-this.meterAt>100){ this.meterAt=now; hud.rate(ctx.game,this.hitT,now); } if(this.lockUntil&&now>=this.lockUntil){ this.lockUntil=0; this.unlock(); } },
