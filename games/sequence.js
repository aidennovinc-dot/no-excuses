...
const SQ={ st:'idle', keys:0, seq:[], idx:0, round:0, timers:[], p:0, rounds:[0,0], copied:[0,0], phase:0,
  ...
  build(){ this.keys=sel.secs; this.st='idle'; $('#seq').innerHTML=Array.from({length:this.keys},(_,i)=>`<div class="key" data-k="${i}" data-n="${i+1}"></div>`).join('');
    $('#hud-mode').textContent=`${SCALES[sel.scale].name} · ${this.keys} keys${sel.practice?' · practice':this.mode()==='pass'?' · pass & play':this.mode()==='compose'?' · compose':''}`; $('#score').style.visibility=sel.practice||sel.vs?'hidden':''; $('#seqdone').classList.remove('on'); },
  ...
  play(){ this.clearT(); this.st='play'; const pass=this.mode()==='pass'; $('#score').textContent=this.round-1; $('#hud-time').textContent=pass?`round ${this.round}`:`round ${this.round} · watch`; $('#hud-time').classList.remove('you'); $('#seq').classList.add('watch'); $('#seq').classList.remove('input'); const step=this.step();
    if(pass) this.cue(`${pWho(this.p)}<br><small style="font-size:.6em;letter-spacing:.2em">copy the notes</small>`,true,this.p); else if(this.round<=4&&this.phase===0) this.cue('copy the notes',true);
  ...
  yourTurn(p){ this.st='input'; this.idx=0; $('#hud-time').innerHTML=p===undefined?'your turn':`${pWho(p)} · your turn`; $('#hud-time').classList.add('you'); $('#seq').classList.remove('watch'); $('#seq').classList.add('input'); this.cue(p===undefined?'your turn':`${pWho(p)}<br><small style="font-size:.6em;letter-spacing:.2em">your turn</small>`,false,p); Snd.turn(); },
  press(k){ if(this.st==='compose') return this.composeTap(k); if(this.st!=='input') return;
    if(k===this.seq[this.idx]){ this.light(k,700,180); this.idx++; if(this.idx===this.seq.length){ this.st='wait'; $('#seq').classList.remove('input'); $('#hud-time').classList.remove('you'); Snd.hit();
      ...
      if(this.mode()==='pass'){ const w=1-this.p; this.cue(`${pWho(w)} wins`,true,w); return this.later(()=>finish({hits:this.rounds[0],misses:0,vs2:{a:this.rounds[0],b:this.rounds[1],w,how:'on a missed note'}}),1600); }
  ...
  compose(c){ this.clearT(); this.phaseComposer=c; this.st='compose'; this.seq=[]; $('#seq').classList.remove('watch','input'); $('#hud-time').innerHTML=`${pWho(c)} · compose · up to 8`; this.cue(`${pWho(c)}<br><small style="font-size:.6em;letter-spacing:.2em">tap in a tune · up to 8 notes</small>`,true,c); $('#seqdone').classList.remove('on'); },
  composeTap(k){ if(this.seq.length>=8) return; this.seq.push(k); this.light(k,600,200); $('#hud-time').innerHTML=`${pWho(this.phaseComposer)} · ${this.seq.length} of 8`; if(this.seq.length>=1) $('#seqdone').classList.add('on'); if(this.seq.length>=8) this.later(()=>this.done(),500); },
    ...
    this.cue(`${pWho(p)}<br><small style="font-size:.6em;letter-spacing:.2em">listen · then copy it</small>`,true,p); $('#hud-time').innerHTML=`${pWho(p)} · listen`;
  ...
  swap(){ $('#seq').classList.remove('input','watch'); $$('.key').forEach(k=>k.classList.remove('bad','lit')); if(this.phase===0){ this.phase=1; return this.compose(1); }
    const a=this.copied[0], b=this.copied[1], w=a>b?0:b>a?1:-1; this.cue(w<0?'draw':`${pWho(w)} wins`,true,w<0?undefined:w); this.later(()=>finish({hits:a,misses:0,vs2:{a,b,w,how:'on the longer copy',txt:[a+' notes',b+' notes']}}),1600); } };
