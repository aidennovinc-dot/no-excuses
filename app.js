...
// the engines by game id. Code, not data — it stays here until Stage 3's games/registry.js exports ENGINES
...
const VX={ n:[0,0], tgt:[0,0], lock:[0,0], streak:[0,0], pos:[null,null], next:[null,null], raf:0, t0:0, done:false,
  ...
  setup(){ this.n=[0,0]; this.lock=[0,0]; this.streak=[0,0]; this.done=false; $('#vn0').textContent='0'; $('#vn1').textContent='0';  $('#vsdiff').textContent=T(HUD.vsLead,{n:VS_LEAD}); $('#vsnote').textContent=this.qt()?HUD.vsQt:HUD.vsDots; $('#vslead').style.left='50%'; $('#vslead').style.width='0'; $('#vwin').classList.remove('on');
  ...
  score(p){ this.n[p]++; const el=$('#vn'+p); el.textContent=this.n[p]; Snd.hit(); const d=this.n[0]-this.n[1]; const k=Math.min(1,Math.abs(d)/VS_LEAD)*50; const bar=$('#vslead'); bar.style.width=k+'%'; bar.style.left=d>=0?'50%':(50-k)+'%'; $('#vsdiff').innerHTML=d===0?HUD.level:T(HUD.lead,{who:pWho(d>0?0:1),n:Math.abs(d)});
  ...
  end(){ if(this.done) return; this.done=true; cancelAnimationFrame(this.raf); const a=this.n[0], b=this.n[1]; const w=a>b?0:b>a?1:-1; const win=$('#vwin'); win.innerHTML=w<0?`<div>${HUD.draw}</div>`:`<div class="${w?'top p2':'p1'}">${T(HUD.wins,{n:w+1})}</div>`; win.classList.add('on'); Snd.end();
    G.timers.push(setTimeout(()=>finish({hits:a,misses:0,vs2:{a,b,w,how:Math.abs(a-b)>=VS_LEAD?T(HUD.byLead,{n:VS_LEAD}):HUD.onClock}}),1900)); } };
...
export { Ads, DRAW, ENGINE, H, Intro, VX, W, abort, arm, countdown, cv, cx, dpr, finish, goWhere, hit, liveCheck, miss, pbShow, peakRate, pts, rateMeter, reduce, size, start, tapAt, tick };
