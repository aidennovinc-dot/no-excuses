...
- **FOUR chests in a column on game select, named by what opens them — Games, Key, Pro, Thorns — never numbered** (v23 L.10, build 40, amending v18 B.19 / B.20 and v21 G.1–G.4): `config/chests.js`; Games needs every mode (it REPLACED G.3's gate), Key needs key 1 whole, Pro needs Pro, Thorns needs Author; strictly sequential — `chestState()` in `progress/key.js`, and nothing is ready behind a shut chest (gated). A chest tap opens its key screen, and a READY chest opens THERE by itself (L.8b — "Open the chest?" and the step into Pro are retired); plain lid-up in build 40, the ceremony is build 41. An opened chest shows what it gave as words beside it (`CHEST_WORDS`, L.11c). Each game tile's outline fills with its key-1 progress in `KEYFILL` lilac (B.18) — from the Games chest on.
