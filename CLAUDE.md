...
- **Three chests in a column on game select** (v18 B.19 / B.20 / B.16): chest n needs key n whole; a ready chest ASKS ("Open the chest?") and the build-29 opening is extended, not replaced; an opened chest 1 or 2 offers the step into the next tier. **Since build 37 (v21 G.1, amending B.19, quoting v17 §A.1) chests 2 and 3 are ON THE MAP from the start, stacked and locked, each saying "open the previous chest"** — and chest 1 also waits for every game mode (G.3). Each game tile's outline fills with its key-1 progress in `KEYFILL` lilac (B.18), complete is a wash as well as a closed line.
- **v17 §A.1 hides NUMBERS, not existence** (v21 G.1 / G.2, v20 D.7, build 37): every chest and all three keys are on screen from the first visit; a key whose tier is shut is crossed out with "To unlock: open the previous chest", carries no %, and opens nothing. **Each tier opens with its own chest (build 38, Aiden): Pro with chest 1, Author with the Pro chest** — `tierOpen()` in `progress/key.js` is the one line, and the key strip, clears, retroactive credit, the key achievement sets and the radar's rungs all ask it.
- **A chest opening credits the tiers it reveals SILENTLY** (v21 G.4): `retroBank()` banks every newly revealed bar a saved best already beats — no toast, no unlock sound; `prefs.retro` puts L8's green on those rows once. A live clear still interrupts the result screen. **A column that ARRIVES for a tier already open is credited the same way, once** (#426, Aiden: "yes, silently, once"): `retroArrived()` at boot, keyed by `prefs.retroCol[tier]` — the column as last credited — so a reload never credits twice and a real number replacing a placeholder credits once more.
- **The front % counts up when it has risen since last painted** (v20 D.4): `prefs.pctSeen[tier]`, written on paint; the count-up is `core/count.js`, shared with the runs (A4), and the sound is its own whoosh.
- **Testing has a switch and a reset per key** (v21 G.8, S5): the switch snapshots and restores, the reset backs the key out — bars, chest, tier step, retro marks, key achievements.
...
- **Customise's music is ONE row, called Music, and it is the track** (v18 B.28): the buttons are this game's three tracks by name, a tap plays one, and until chest 2 (A.3) the row is the single track it is set to behind a padlock, saying nothing about what opens it (A.1). No on / off, no Preview button — the per-game off switch went with them. The menu loop keeps its own.
...
- **The game-select grid is a snake placed from `Object.keys(GAMES)`** by `grid-row` / `grid-column`; its lines are measured by `offsetLeft` / `offsetTop`, never a bounding rect; the chest is the last stop; since build 37 the Pro and Author chests sit under it from the start, locked, and nothing about their NUMBERS shows before chest 1 is opened (A.1, narrowed by v21 G.1). While any mode is locked a gate sits on the connector into chest 1 (G.3).
