...
lives beside `BUILD` and does **not** move with it (it is 2; the comment there says why).
...
- **Every mode offers Set and Streak** — Set a fixed number of rounds, Streak until you fail (L5, `SET_COPY` in `config/games.js`).
...
- **A running total says what it is measuring** — unit, meaning, and where there is a budget, what is spent and what the budget is (v17 B.1).
...
- **THREE keys are difficulty tiers over the same combinations**; keys 2 and 3 are `shell:true` in `config/keys.js` until #372 decides, and A.2 forbids deriving a bar. Key 1 carries a percentage (`keyPct()` in `progress/key.js`) on the keys screen and the menu; keys 2 and 3 show nothing before chest 1 (A.1).
...
- **Flow state is one number with two consumers**: the engine's `tps()` → `R.flow` / `--flow` in `run/run.js` → the hum in `audio.js`. Solo Quick Tap and Dots only, light blue (L4), presentation only (L10). Sequence ducks its bed to 40% while a key rings.
...
`core/store.js`. A new `prefs` field goes into `cleanPrefs` in the commit that adds it; Fresh game clears progress and
keeps preferences; `runs` is capped at 600; dev switches exist only while `BUILD_FLAGS.dev` is true (S5).
...
| L5 | Every mode offers Set and Streak. **Streak = a cumulative budget** — Estimate 100%, Stopwatch 25s (30s past round 10), Hidden 100px, Flash 500ms over 150 (an early tap spends 400ms flat and the attempt), Go / No-go 1000ms over 150 (a wrong tap costs 200ms), Count 5 miscounts, Find 10s; score = rounds completed, "Highest round wins!", no wrong-tap run-ender. **Set = a fixed number of rounds, scored by the line on the sheet**; Go / No-go's Set adds 150ms per wrong tap and ends on three; the currencies are never harmonised. Counts and lines come from `SET_COPY` in `config/games.js`. |
| L6 | The unlock chain and thresholds are the §1 table in the latest FEEDBACK file that names L6. One table — `UNLOCKS` + `LEN_RULES` in `config/unlocks.js`, predicates in `progress/rules.js`, keyed `'game:mode'` — feeds lock boxes, goal lines, the Next card and the Unlocks tab; every requirement names its game; `lenNeed(g,d,s)` in `progress.js` builds every requirement sentence. Sequence is 3 and 7 keys, opened by one Cut round within 3.5%; Quick Tap · Four opens from any Two run; Cut's Streak asks for more than 10% off; "N hits, no misses" is N in a row; five rows ask the player to fail on purpose, and tapping a locked row to read its requirement is part of the rule; a length unlock announces. |
