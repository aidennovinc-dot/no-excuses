...
| L5 | Every mode offers Set and Streak. **Streak = a cumulative budget** — Estimate 100%, Stopwatch **5s (7.5s past round 10)**, Hidden **700ms**, Flash 500ms over 150 (an early tap spends 400ms flat and the attempt), Go / No-go 1000ms over 150 (a wrong tap costs 200ms), Count 5 miscounts, Find 10s; score = rounds completed, "Highest round wins!", no wrong-tap run-ender. **Set = a fixed number of rounds, scored by the line on the sheet** — Stopwatch and Hidden are **totals**, Hidden in **milliseconds**; a Flash attempt over **1000ms** scores 1000ms and counts; Go / No-go's Set is **5 rounds of 3 correct taps** and adds 150ms per wrong tap, **with no run-ender at all**; the currencies are never harmonised. Counts and lines come from `SET_COPY` in `config/games.js`. |
