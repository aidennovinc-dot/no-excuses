...
- **v17 §A.1 hides NUMBERS, not existence** (v21 G.1 / G.2, v20 D.7, build 37): every chest and all three keys are on screen from the first visit; before `mapOpen()` Pro and Author keys are crossed out with "To unlock: open the previous chest", carry no %, and open nothing. Their rings, bars, achievement sets and radar rungs still arrive with chest 1.
- **A chest opening credits the tiers it reveals SILENTLY** (v21 G.4): `retroBank()` banks every newly revealed bar a saved best already beats — no toast, no unlock sound; `prefs.retro` puts L8's green on those rows once. A live clear still interrupts the result screen.
...
- **One colour for "this is what you chose"** (v22 §K): a selected mode takes `--press`; while the sheet is up the pressed tile demotes to the line colour; `.picked` keeps `--ok`. The game-then-mode pick only — `.sel` elsewhere is unchanged.
