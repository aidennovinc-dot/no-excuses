...
import { CHEST_LOOK, METER_BANDS, SPILL } from "../config/chests.js";
...
function wordsHtml(id) { return (CHEST_WORDS[id] || []).map((x, i) =>
  `<button class="cw${x.tba ? ' tba' : ''}" data-act="chestword" data-for="${id}" data-to="${esc(x.to || 'soon')}" data-w="${esc(x.w)}" style="--i:${i}">${esc(x.w)}${x.tba ? `<small>${esc(GRID.tba)}</small>` : ''}</button>`).join(''); }
...
export { burstHtml, chestSvg, meterLook, spillVars, wordsHtml };
