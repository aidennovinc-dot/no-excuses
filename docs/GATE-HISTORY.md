> **Frozen at build 47.** This is `_smoke/GATE.md` as it stood at build 46, moved here untouched by the gate housekeeping build:
> what each assertion from build 14 to build 46 was written to stand for, paragraph by paragraph. Nothing is appended here any more —
> `_smoke/GATE.md` is now an index, one line per section, and a new section adds one line there.
